  ...
  return (
    <section className="w-full py-12 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          <h2 className="text-4xl font-bold text-gray-800 dark:text-white">
            {t("topAttraction.title")}
          </h2>
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            {t("topAttraction.subtitle")}
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <button
            onClick={prevSlide}
            disabled={currentIndex === 0}
            aria-label={t("topAttraction.prev")}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20
                     h-12 w-12 rounded-full bg-white shadow-lg
                     flex items-center justify-center
                     hover:scale-110 transition-all
          ...
          <button
            onClick={nextSlide}
            disabled={currentIndex === maxIndex}
            aria-label={t("topAttraction.next")}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-20
                     h-12 w-12 rounded-full bg-white shadow-lg
                     flex items-center justify-center
                     hover:scale-110 transition-all
              ...
              {attractions.map((place) => (
                ...
                >
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden hover:shadow-xl transition h-full">
                    <img
                      src={`${place.imageUrl}?w=600&auto=format&fit=crop`}
                      alt={t(`topAttraction.items.${place.key}.name`)}
                      loading="lazy"
                      decoding="async"
                      className="w-full h-52 object-cover"
                    />
                    ...
                    <div className="p-4">
                      <h3 className="text-lg font-semibold mb-1 text-gray-800 dark:text-white">
                        {t(`topAttraction.items.${place.key}.name`)}
                      </h3>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                        {t(`topAttraction.items.${place.key}.description`)}
                      </p>
          ...
          <div className="flex justify-center gap-2 mt-6">
            {Array.from({ length: maxIndex + 1 }).map((_, index) => (
              <button
                key={index}
                onClick={() => setCurrentIndex(index)}
                aria-label={`${t("topAttraction.goToSlide")} ${index + 1}`}
                className={`h-2.5 w-2.5 rounded-full transition-all ${
                  currentIndex === index ? "bg-red-600 w-6" : "bg-gray-300"
                }`}
              />
